...
def build_help_from_template(content_file, template_file, id, title, button_title, button_icon):
    # Read the content from the first HTML file
    print("Reading content from file: %s" % content_file)
    with open(content_file, 'r', encoding='utf-8') as f:
        content = f.read()

    # Read the template HTML file
    #print("Reading template from file: %s" % template_file)
    with open(template_file, 'r', encoding='utf-8') as f:
        template = f.read()

    # Replace the placeholders with the provided content
    updated_template = re.sub(r'\$HELP_MODAL\$', id, template) 
    updated_template = updated_template.replace('$HELP_CONTENT_TITLE$', title)
    updated_template = updated_template.replace('$HELP_BUTTON_TITLE$', button_title)
    updated_template = updated_template.replace('$HELP_BUTTON_ICON$', button_icon)

    # Replace the placeholder with the content
    updated_template = updated_template.replace('$HELP_MODAL_CONTENT$', content)

    return updated_template

def convertToHtml(inputFileName, outputFileName, templateFileName=None, iframe=False, topFolder="..",
                  docFolder=".", footerFileName='footer.html', navFileName='', removeMermaid=False):
    ...
    if not iframe:
        ...
        if inputFileName.endswith('.md'):            
            ...
            print('- Convert markdown to html', len(html))
        else:
            html = ''
    ...
    print('------ Write to file', outputFileName)
    print(html)
    print('------ END Write to file', outputFileName)
...
def getOutputFileName(inputFileName, outputPath, inputExtension, outputExtension):
    ...
    print('inputExtension: %s' % inputExtension)
    print('outputExtension: %s' % outputExtension)

    returName = ''
    if outputPath:
        returnName = os.path.join(outputPath, os.path.basename(inputFileName).replace(inputExtension, outputExtension))
    else:
        returnName = inputFileName.replace(inputExtension, outputExtension)    

    print('returnName: %s' % returnName)
    return returnName
...
def main():
    ...
    parser.add_argument('-p', '--popup', type=bool, default=False, help="Create for popup")
